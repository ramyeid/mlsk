...
public class OrchestratorImpl implements Orchestrator {
  ...
  @Override
  public <Result> Result priorityRunOnEngine(int engineId, String actionName, Function<Engine, Result> action) {
    Engine engine = this.engines.get(engineId);
    if (engine == null) {
      throw new NoEngineWithInformationException(format("No engine with id: `%s` to run `%s`", engineId, actionName));
    }
    LOGGER.info("Running priority action: `{}` on engine: `{}`", actionName, engineId);
    return action.apply(engine);
  }

  @Override
  public <Result> List<Result> priorityRunOnAllEngines(String actionName, Function<Engine, Result> action) {
    return this.engines.keySet()
        .stream()
        .map(id -> priorityRunOnEngine(id, actionName, action))
        .collect(toList());
  }
