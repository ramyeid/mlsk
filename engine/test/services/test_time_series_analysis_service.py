...
class TestTimeSeriesAnalysisService(unittest.TestCase):
  ...
  def test_predict_service(self):
        #Given
    initial_data = {"Date": [ datetime.strptime("1960-08-01 11:00:00", "%Y-%m-%d %H:%M:%S") + timedelta(hours=i) for i in range(0, 14) ], 
                    "Value": [ 112.0, 118.0, 132.0, 129.0, 121.0, 135.0, 148.0, 148.0, 136.0, 119.0, 104.0, 118.0, 115.0, 126.0 ]}
    ...
    #Then
    data_with_predicted_values = {"Date": [ datetime.strptime("1960-08-01 11:00:00", "%Y-%m-%d %H:%M:%S") + timedelta(hours=i) for i in range(0, 17) ],
                                  "Value": [ 112.0, 118.0, 132.0, 129.0, 121.0, 135.0, 148.0, 148.0, 136.0, 119.0, 104.0, 118.0, 115.0, 126.0, 
  ...
  def test_forecast_service(self):
    #Given
    initial_data = {"Date": [ datetime.strptime("1960-08-01 11:00:00", "%Y-%m-%d %H:%M:%S") + timedelta(days=i) for i in range(0, 14) ], 
                    "Value": [ 112.0, 118.0, 132.0, 129.0, 121.0, 135.0, 148.0, 148.0, 136.0, 119.0, 104.0, 118.0, 115.0, 126.0 ]}
    tsa = TimeSeriesAnalysisService(pd.DataFrame.from_dict(initial_data), "Date", "Value", 3)
    ...
    #Then
    data_with_forecasted_values = {"Date": [ datetime.strptime("1960-08-01 11:00:00", "%Y-%m-%d %H:%M:%S") + timedelta(days=i) for i in range(0, 17) ], 
                                  "Value": [ 112.0, 118.0, 132.0, 129.0, 121.0, 135.0, 148.0, 148.0, 136.0, 119.0, 104.0, 118.0, 115.0, 126.0, 
  ...
  def test_compute_forecast_accuracy(self):
    #Given
    initial_data = {"Date": [ datetime.strptime("1960-08-01 11:00:00", "%Y-%m-%d %H:%M:%S") + timedelta(days=i) for i in range(0, 17) ], 
                    "Value": [ 112.0, 118.0, 132.0, 129.0, 121.0, 135.0, 148.0, 148.0, 136.0, 119.0, 104.0, 118.0, 115.0, 126.0, 141.0, 135.0, 125.0 ]}
    tsa = TimeSeriesAnalysisService(pd.DataFrame.from_dict(initial_data), "Date", "Value", 3)
